...
# ===============================
# SYMPTOM & DISEASE MAPS
# ===============================
symptoms_dict = {
    'itching': 0, 'skin_rash': 1, 'nodal_skin_eruptions': 2, 'continuous_sneezing': 3,
    'shivering': 4, 'chills': 5, 'joint_pain': 6, 'stomach_pain': 7,
    'acidity': 8, 'ulcers_on_tongue': 9, 'muscle_wasting': 10, 'vomiting': 11,
...
diseases_list = {
    ...
    13: 'Dimorphic hemmorhoids(piles)', 14: 'Drug Reaction',
    ...
    30: 'Migraine', 31: 'Osteoarthristis',
    32: 'Paralysis (brain hemorrhage)', 33: 'Peptic ulcer diseae',
...
def get_predicted_value(patient_symptoms):
    ...
    return prediction_index, input_vector
...
def home():
    if request.method == 'POST':
        ...
        prediction_index, input_vector = get_predicted_value(user_symptoms)

        if prediction_index is None:
            return render_template('index.html', message="❌ No valid symptoms detected")

        # ✅ CONFIDENCE CHECK (even for 1 symptom)
        confidence = np.max(np.abs(svc.decision_function([input_vector])))

        if confidence < 0.4:
            return render_template(
                'index.html',
                message="⚠ Unable to confidently predict disease. Please consult a doctor."
            )
